...
def data_segmentation(data_path, target_path, t):
	...
	trainData, validData, testData =  \
			data[rnd_idx[1:trBatch],:], \
			data[rnd_idx[trBatch+1:trBatch + validBatch],:], \
			data[rnd_idx[trBatch + validBatch+1:-1],:] \

	trainTarget, validTarget, testTarget = \
			target[rnd_idx[1:trBatch], task], \
...
# Select gender instead of name?
use_genders = False

trainData, validData, testData, trainTarget, validTarget, testTarget = \
		data_segmentation('data.npy', 'target.npy', use_genders)
...
print("Number percent error in the test set when k = %d is %f" \
		 % (minK, sess.run(error)) )
