...
def Simulation(index):
    ...
    learningRate = np.exp(np.random.choice(learningRateOptions))
    dropout = np.random.choice(dropOutOptions)
    numLayers = np.random.choice(numLayerOptions)
    numHiddenUnits = np.random.choice(numHiddenUnitsOptions)
    weightDecay = np.exp(np.random.choice(weightDecayOptions))
